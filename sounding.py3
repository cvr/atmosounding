...
## import functions from local .py3 script
import imp
savgol_irregular_brute = imp.load_source("savgol_irregular_brute", "savitzky_golay_lowpassfilter.py3").savgol_irregular_brute
savgol_irregular_interp = imp.load_source("savgol_irregular_interp", "savitzky_golay_lowpassfilter.py3").savgol_irregular_interp
...
def read_sounding (fname):
    ...
    fid = open(fname, 'Ur',  errors='replace')
    lines = fid.readlines()
    fid.close()
    ## get station name
...
lbl_Fr_signed = r"$Fr^2/\sqrt{|Fr^2|}$"
